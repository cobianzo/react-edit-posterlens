...
//    console.log(retrievedOptions.worlds[0].hotspots[7].rot);
  ...
  // sync localstorage with pl options. This is not working. Only happens on mount.
  // useEffect(() => { 
  //   if (!pl) return;
  //   console.log('UseEffect: updating localstorage with worldoptions', );
  //   // exportOptions('localStorage')        
  //   var exportStr = JSON.stringify(pl.o, false, 2);
  //   pl.Modal('Params', exportStr)
  // }, [pl]);

  // NOT WORKING: se peta . get info from all hotspots and save it into pl.
  function syncViewerParams() {

    const worldParams = getCurrentPanoramaParams();
    window.pl.viewer.panorama.children.forEach( ob => {
      if ( ob.type.substring(0,3) !== 'pl_' ) return;
        singleObject3DToParams(ob);
    }) 
  }
